    ...
    type PGNiGClient(username, password) =
        ...
        member this.ProcessInvoices(invoices:list<array<IElementHandle> * array<IElementHandle>>) =
            ...
            |> List.map (fun (a,b) ->
                ...
                // sample text - "Adres:  80-433  Gdańsk, ul. Ludwika Waryńskiego 46 B /6"
            ...
            |> List.map (fun (a, d, i) ->
                let curve =
                    ...
                    | "Adres:80-251Gdańsk,StefanaBatorego10/5"
                    | "Adres:80-251Gdańsk,ul.StefanaBatorego10/5"     -> "batorego 10/5 gas invoice-total-amount a"
                    | "Adres:80-433Gdańsk,LudwikaWaryńskiego46B/6"
                    | "Adres:80-433Gdańsk,ul.LudwikaWaryńskiego46B/6" -> "warynskiego 46b/6 gas invoice-total-amount a"
                curve, d, i
            )
        ...
        member this.ScrapeOverpayments() =
            ...
            // JSHandle:010/2021/157244/UI54563288018590365500027859174 010/2021/157244/UIFILIP KUCHARCZYK Stefana Batorego 10 /5, 80-251 Gdańsk19,65 zł
            // JSHandle:010/2019/5010/UI8265608018590365500021566160 010/2019/5010/UIKUCHARCZYK FILIP ul. Ludwika Waryńskiego 46 B /6, 80-433 Gdańsk
            ...
            let parseRow text =
                ...
                let m = Regex.Match(text, "JSHandle:.*(?:FILIP KUCHARCZYK|KUCHARCZYK FILIP)(.*?)Gdańsk(.*)")
            ...
            let nameToCurveMap x =
                match x with
                | "Stefana Batorego 10 /5, 80-251"          -> "batorego 10/5 gas account-overpayment a"
                | "ul. Ludwika Waryńskiego 46 B /6, 80-433" -> "warynskiego 46b/6 gas account-overpayment a"
